...
def update(frame):
    ...
    # 更新y轴范围
    max_product1 = max([max(p) for p in products1])
    ax1.set_ylim(0, max(max_product1, 1.5))
    ax2.set_ylim(0, max(overlap_areas1) * 1.1)
    ...
    # 更新y轴范围
    max_product2 = max([max(p) for p in products2])
    ax3.set_ylim(0, max(max_product2, 1.5))
    ax4.set_ylim(0, max(overlap_areas2) * 1.1)
